...
do
	...
	then
		...
		--log_dir log_dir --eval_batch_size 640 --val_path $dataset/trn.json \
		...
		--max_seq_length 430 --max_label_length 15 \
		--output_shortlist_path $dataset/trn_shortlist_$epoch.json \
		--model_path $dataset/model_dir/biencoder_iter_$model_name_epoch.pt
	fi

	echo "epoch {$epoch} shortlist finished"

	python src/inference_wizardlm_batch.py \
	--trn_docs_path $dataset/trn.json \
	...
	--topk $llm_topk --batch_size $batch_size_llm --max_seq_length 430 \
	--prompt_id $prompt_id
	...
	python src/get_shortlist.py --share_weight \
		...
		--model_path $dataset/model_dir/biencoder_iter_$epoch.pt
	...
	--topk 2 --batch_size $batch_size_llm --max_seq_length 430 --prompt_id $prompt_id
